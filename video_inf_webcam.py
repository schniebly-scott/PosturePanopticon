...
try:
    ...
    cam_index = args.input

cap = cv2.VideoCapture(cam_index)
frame_width = int(cap.get(3))
frame_height = int(cap.get(4))
video_fps = int(cap.get(5)) if int(cap.get(5)) > 0 else 30

# Optional: only write video if input is a file
write_video = not isinstance(cam_index, int)
if write_video:
...
# detect_objects, detect_pose, draw_keypoints
